...
def build_mission_definition(env: MultiUAVParallelEnv, config: dict) -> dict:
    ...
    top_left = (base_env.max_lat, base_env.min_lon)
    bottom_right = (base_env.min_lat, base_env.max_lon)
    ...
    return {
        ...
        "episode_max_steps": int(base_env.max_steps),
        "base_max_steps": int(base_env.base_max_steps),
        "timeout_scaled": bool(base_env.max_steps != base_env.base_max_steps),
        "timeout_max_route_distance_m": float(base_env._episode_timeout_max_route_distance_m),
        "timeout_reference_route_distance_m": float(
            base_env._episode_timeout_reference_route_distance_m
        ),
        "waypoints_per_drone": (
    ...
    *,
    ...
    summary = {
        ...
        "waypoint_throughput_per_min": (
            float(reached_waypoints / max((steps * dt) / 60.0, 1e-6)) if steps > 0 else 0.0
        ),
        "avg_distance_per_uav_m": avg_distance,
        "total_distance_m": float(total_distance),
        "planned_route_distance_m_total": float(mission_definition["planned_route_distance_m_total"]),
        ...
        "min_pairwise_distance_m": float(min_pairwise_distance) if min_pairwise_distance is not None else None,
...
def build_empty_history() -> dict:
    from collections import Counter
    return {
        "rewards": [], "steps": [], "durations_s": [], "reward_per_step": [],
        "wps": [], "throughput": [], "distances": [], "distance_ratios": [],
        "caution_counts": [], "critical_counts": [], "geofence_counts": [], "outside_steps": [],
        "deconfliction_times": [],
        "min_pairwise_distances": [], "crashes": 0, "failed": 0, "completed": 0,
        "max_steps": 0, "timed_out": 0, "boundary_compliant": 0,
...
def update_history(history: dict, mission_record: dict):
    ...
    history["distance_ratios"].append(float(summary["distance_vs_planned_ratio"]) if summary["distance_vs_planned_ratio"] is not None else np.nan)
    ...
    history["min_pairwise_distances"].append(float(summary["min_pairwise_distance_m"]) if summary["min_pairwise_distance_m"] is not None else np.nan)
...
def build_benchmark(history: dict) -> dict:
    ...
    return {
        ...
        "avg_deconfliction_time_s": float(deconfliction_times.mean()) if deconfliction_times.size else 0.0,
        "avg_distance_vs_planned_ratio": float(finite_distance_ratios.mean()) if finite_distance_ratios.size else None,
        "avg_min_pairwise_distance_m": float(finite_min_pairwise.mean()) if finite_min_pairwise.size else None,
        "status_counts": dict(history["status_counts"]),
        "num_missions": int(len(rewards)),
    }


def print_benchmark(summary: dict, *, title: str = "MAPPO Evaluation Summary"):
    ...
    table.add_row("Avg Dist/Planned", str(summary["avg_distance_vs_planned_ratio"]))
