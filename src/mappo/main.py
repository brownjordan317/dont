...
class CategoricalSkillActor(nn.Module):
    def __init__(
        self,
        obs_dim: int,
        hidden_sizes: Iterable[int],
        ...
        self.backbone = build_mlp(obs_dim, hidden_sizes, feature_dim)
        self.logits_head = nn.Linear(feature_dim, self.action_dim)
    ...
    def sample(self, obs: torch.Tensor, deterministic: bool = False):
        ...
        action = F.one_hot(skill, num_classes=self.action_dim).to(dtype=torch.float32)
        log_prob = dist.log_prob(skill)
        entropy = dist.entropy()
        return action, action, log_prob, entropy
    ...
    def evaluate_actions(self, obs: torch.Tensor, raw_action: torch.Tensor):
        ...
        if raw_action.ndim == 1:
            skill = raw_action.long()
        elif raw_action.shape[-1] == 1:
            skill = raw_action.squeeze(-1).long()
        else:
            skill = torch.argmax(raw_action, dim=-1).long()
        action = F.one_hot(skill, num_classes=self.action_dim).to(dtype=torch.float32)
        log_prob = dist.log_prob(skill)
        entropy = dist.entropy()
...
class CentralValueCritic(nn.Module):
    def __init__(
        ...
        self.network = build_mlp(state_dim, hidden_sizes, self.value_dim)
...
class MAPPOPolicy:
    def __init__(
        ...
        *,
        ...
        actor_type: str = "categorical_skill",
        critic_value_dim: int = 1,
    ):
        ...
        self.actor_type = str(actor_type).strip().lower()
        self.critic_value_dim = max(int(critic_value_dim), 1)
        ...
        elif self.actor_type == "categorical_skill":
            self.actor = CategoricalSkillActor(
                obs_dim=self.obs_dim,
                hidden_sizes=self.actor_hidden_sizes,
                action_dim=self.action_dim,
            ).to(self.device)
        ...
        self.critic = CentralValueCritic(
            ...
            value_dim=self.critic_value_dim,
        ).to(self.device)
    ...
    def act_parallel(
        ...
        *,
        ...
        with torch.no_grad():
            ...
            values_np = self.critic(states_tensor).cpu().numpy()
            if self.critic_value_dim == 1:
    ...
    def save(
        ...
        *,
        ...
        payload = {
            ...
            "critic_value_dim": self.critic_value_dim,
    ...
    def load(cls, path: str, device: str = "cpu") -> "MAPPOPolicy":
        ...
        if actor_type is None:
            actor_type = "gaussian" if "log_std" in actor_state_dict else "categorical_skill"
        policy = cls(
            ...
            critic_value_dim=int(
                payload.get(
                    "critic_value_dim",
                    cls._infer_critic_value_dim(payload["critic_state_dict"]),
                )
            ),
        )
    ...
    @staticmethod
    def _infer_critic_value_dim(critic_state_dict: dict) -> int:
        for key in reversed(list(critic_state_dict.keys())):
            value = critic_state_dict[key]
            if key.endswith(".bias") and hasattr(value, "numel"):
...
def build_rollout_batch(
    *,
    ...
    if rewards_arr.ndim == 1:
        rewards_arr = np.repeat(rewards_arr[:, None], masks_arr.shape[1], axis=1)
    elif rewards_arr.ndim != 2:
        raise ValueError(
            "Rollout rewards must be scalar per step or a per-agent matrix; "
            f"got shape {rewards_arr.shape}"
        )
    if rewards_arr.shape != masks_arr.shape:
        raise ValueError(
            "Reward/mask shape mismatch: "
            f"rewards={rewards_arr.shape}, masks={masks_arr.shape}"
        )

    active_counts = np.maximum(masks_arr.sum(axis=1), 1.0)
    team_rewards_arr = (rewards_arr * masks_arr).sum(axis=1) / active_counts
    values_arr = np.asarray(values, dtype=np.float32)
    dones_arr = np.asarray(dones, dtype=np.float32)
    if values_arr.ndim == 1:
        advantages, returns = compute_gae(
            rewards=team_rewards_arr,
            values=values_arr,
            dones=dones_arr,
        ...
        agent_advantages = np.zeros_like(rewards_arr, dtype=np.float32)
        for agent_idx in range(rewards_arr.shape[1]):
            agent_advantages[:, agent_idx], _ = compute_gae(
                rewards=rewards_arr[:, agent_idx],
                values=values_arr,
                dones=dones_arr,
                bootstrap_value=bootstrap_value,
                gamma=gamma,
        ...
        if values_arr.shape != masks_arr.shape:
            raise ValueError(
                "Per-agent critic value/mask shape mismatch: "
                f"values={values_arr.shape}, masks={masks_arr.shape}"
            )
        bootstrap_arr = np.asarray(bootstrap_value, dtype=np.float32)
        if bootstrap_arr.ndim == 0:
            bootstrap_arr = np.repeat(float(bootstrap_arr), masks_arr.shape[1])
        if bootstrap_arr.shape != (masks_arr.shape[1],):
            raise ValueError(
                "Per-agent bootstrap value shape mismatch: "
                f"bootstrap={bootstrap_arr.shape}, expected={(masks_arr.shape[1],)}"
            )
        agent_advantages = np.zeros_like(rewards_arr, dtype=np.float32)
        returns = np.zeros_like(rewards_arr, dtype=np.float32)
        for agent_idx in range(rewards_arr.shape[1]):
            agent_advantages[:, agent_idx], returns[:, agent_idx] = compute_gae(
                rewards=rewards_arr[:, agent_idx],
                values=values_arr[:, agent_idx],
                dones=dones_arr,
                bootstrap_value=float(bootstrap_arr[agent_idx]),
                gamma=gamma,
                gae_lambda=gae_lambda,
            )
    else:
        raise ValueError(
            "Rollout critic values must be scalar per step or a per-agent matrix; "
            f"got shape {values_arr.shape}"
        )
    return RolloutBatch(
        obs=np.asarray(obs, dtype=np.float32),
        states=np.asarray(states, dtype=np.float32),
        ...
        advantages=agent_advantages.astype(np.float32),
    )
